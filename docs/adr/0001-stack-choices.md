...
| Mobile | **Expo / React Native** (run via Expo Go) | Flutter, PWA |
| Remote access | **API serves the web board + Tailscale mesh** | Cloud-hosted API; opening a port with port-forwarding |
...
story. This edition runs the app through the **Expo Go** client (no standalone binary to build yet).

**Serve the web board from the API, reach it over Tailscale.** Because the printer pins the backend
to one PC, remote use means reaching that PC, not moving to the cloud. Mounting the built React board
on the API (`StaticFiles`) gives a browser-based client for free at the API's own address, and a
private **Tailscale** mesh makes that address reachable from a phone anywhere without opening a port
or standing up any cloud infrastructure. A cloud-hosted API was rejected because it can't own the
printer; raw port-forwarding was rejected because it exposes the box to the public internet.
...
- **The API must run where the printer is.** Real printing requires the backend on the PC with the
  RP850; other machines and phones are clients over the LAN, or over a Tailscale mesh for remote use.
  The Dockerfile therefore defaults to `PRINT_MODE=console`.
